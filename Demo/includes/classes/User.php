...
class User {
  ...
  // I will remove this person from two arrays, array of person who loggedIn, and the persond who removed
  public function removeFriend($user_to_remove) {

		$logged_in_user = $this->user['username'];

		// The list of friends for the person who i will delete.

		$query = mysqli_query($this->con, "SELECT friend_array FROM users WHERE username='$user_to_remove'");
		$row = mysqli_fetch_array($query);
		$friend_array_username = $row['friend_array']; //The list of friends for the person who i will delete.


		$new_friend_array = str_replace($user_to_remove . ",", "", $this->user['friend_array']); // The list of friends for the person who delete the other guy.
        $remove_friend = mysqli_query($this->con, "UPDATE users SET friend_array='$new_friend_array' WHERE username='$logged_in_user'");

		
        // Update for friends list for the guy whose deleted
